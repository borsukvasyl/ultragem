...
for move_selector, selector_name in zip(
        [worst_move_selector, random_move_selector, best_move_selector, smart_move_selector],
        ['worst', 'random', 'best', 'smart']):
    ...
    print(scores.shape)
    print(selector_name)
    q = scipy.stats.mstats.mquantiles(scores.reshape((Nruns, Nscores * maxswaps)), [0.5, 0.95], axis=0).astype(
        int).reshape((2, maxswaps, Nscores))
