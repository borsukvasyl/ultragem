            ...
            for i in range(ncols):
                if board.type[j, i] == 0 and board.status[j, i] == 0:
                    ...
                    if 0 <= i + left < ncols and self.is_droppable(j - 1, i + left) and board.type[j, i + left] > 0:
                        ...
                        continue
                    elif 0 <= i + right < ncols and self.is_droppable(j - 1, i + right) and board.type[
                        j, i + right] > 0:
...
class PairCombiner(object):
    ...
    def run(self, fromj, fromi, toj, toi):
        ...
        # swap
        self.board.status[toj, toi], self.board.status[fromj, fromi] = self.board.status[fromj, fromi], \
                                                                       self.board.status[toj, toi]
        self.board.type[toj, toi], self.board.type[fromj, fromi] = self.board.type[fromj, fromi], self.board.type[
            toj, toi]
        self.board.color[toj, toi], self.board.color[fromj, fromi] = self.board.color[fromj, fromi], self.board.color[
            toj, toi]
        ...
        if self.board.type[bj, bi] == 5 and self.board.type[aj, ai] == 5:
            ...
            rows, cols = numpy.where(
                numpy.logical_and(numpy.logical_and(self.board.color == color, self.board.type == 1),
                                  self.board.status == 0))
            if type == 4:
